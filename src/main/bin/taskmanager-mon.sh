...
USAGE="Usage: taskmanager-mon.sh (start-mon|start-foreground|stop|stop-all)"
...
if [[ $STARTSTOP == "start-foreground" ]]; then
    ...
    if [[ $FLINK_TM_COMPUTE_NUMA == "false" ]]; then
        ...
        for NODE_ID in "${NODE_LIST[@]:1}"; do
            ...
            numactl --membind=$NODE_ID --cpunodebind=$NODE_ID -- "${FLINK_BIN_DIR}"/flink-daemon-mon.sh $STARTSTOP $ENTRYPOINT "${ARGS[@]}"
        done
    fi
fi
